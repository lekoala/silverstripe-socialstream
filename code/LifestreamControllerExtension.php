...
class LifestreamControllerExtension extends Extension
{
    ...
    public function onAfterInit()
    {
        ...
        if (!empty($whitelist) && $whitelist[0] == '') {
            $whitelist = array();
        }

        if ($config->TwitterUsername && (empty($whitelist) || in_array('twitter',
                $whitelist))) {
            $services[] = '{ service : "twitter", user: "'.$config->TwitterUsername.'" }';
        }
        if ($config->TwitterHashtag && (empty($whitelist) || in_array('twitter_hashtag',
                $whitelist))) {
            $services[] = '{ service : "twitter_hashtag", user: "'.$config->TwitterHashtag.'" }';
        }
        if ($config->FacebookPage && (empty($whitelist) || in_array('facebook_page',
                $whitelist))) {
            $services[] = '{ service : "facebook_page", user: "'.$config->FacebookPage.'" }';
        }
        if ($config->VimeoUsername && (empty($whitelist) || in_array('vimeo',
                $whitelist))) {
            $services[] = '{ service : "vimeo", user: "'.$config->VimeoUsername.'" }';
        }
        if ($config->YoutubeUsername && (empty($whitelist) || in_array('youtube',
                $whitelist))) {
